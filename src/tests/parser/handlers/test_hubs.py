...
from fly_in.parser.handlers.hubs import _parse_hub_fields, handle_start_hub, handle_end_hub, handle_hub


def test_parse_hub_fields_with_all_settings():
...
def test_handle_end_hub_sets_builder_state():
    builder = MapBuilder()
    handle_end_hub(builder, "goal 3 0 [color=red]")
    assert builder.end_hub == "goal"


def test_handle_hub_does_not_set_start_or_end():
    builder = MapBuilder()
